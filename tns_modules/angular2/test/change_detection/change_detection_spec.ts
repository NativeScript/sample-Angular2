...
export function main() {
  describe("change detection", () => {
    StringMapWrapper.forEach({
      ...
      function createChangeDetector(memo, exp, context = null, locals = null, registry = null) {
      ...
      describe(`${name} change detection`, () => {
        ...
        describe("change notification", () => {
          ...
          describe("onChange", () => {
            ...
            it("should notify the dispatcher when a group of records changes", () => {
              var pcd = createProtoChangeDetector();
              var cd = instantiate(pcd, dispatcher, [new BindingRecord(ast("1 + 2"), memo1, dirMemento1), new BindingRecord(ast("10 + 20"), memo2, dirMemento1), new BindingRecord(ast("100 + 200"), memo1, dirMemento2)]);
              cd.detectChanges();
              expect(dispatcher.loggedOnChange).toEqual([{
                'memo1': 3,
                'memo2': 30
              }, {'memo1': 300}]);
            });
            it("should not notify the dispatcher when callOnChange is false", () => {
              var pcd = createProtoChangeDetector();
              var cd = instantiate(pcd, dispatcher, [new BindingRecord(ast("1"), memo1, dirMemento1), new BindingRecord(ast("2"), memo1, dirMementoNoOnChange), new BindingRecord(ast("3"), memo1, dirMemento2)]);
              cd.detectChanges();
          ...
          describe("onAllChangesDone", () => {
            it("should notify the dispatcher about processing all the children", () => {
              ...
              cd.detectChanges();
              expect(dispatcher.loggedValues).toEqual([["onAllChangesDone", memento2]]);
            });
            it("should notify in reverse order so the child is always notified before the parent", () => {
              ...
              cd.hydrate(null, null);
              cd.detectChanges();
              expect(dispatcher.loggedValues).toEqual([["onAllChangesDone", memento2], ["onAllChangesDone", memento1]]);
            });
            it("should notify the dispatcher before processing shadow dom children", () => {
              ...
              var child = shadowDomChildPCD.instantiate(dispatcher, [new BindingRecord(ast("1"), "a", memento)], null, []);
              parent.addShadowDomChild(child);
              parent.hydrate(null, null);
              child.hydrate(null, null);
              parent.detectChanges();
              expect(dispatcher.loggedValues).toEqual([["onAllChangesDone", memento], 1]);
            });
          });
        });
        describe("enforce no new changes", () => {
          it("should throw when a record gets changed after it has been checked", () => {
            ...
            var dispatcher = new TestDispatcher();
            var cd = instantiate(pcd, dispatcher, [new BindingRecord(ast("a"), "a", 1)]);
            cd.hydrate(new TestData('value'), null);
            expect(() => {
              cd.checkNoChanges();
...
class FakeDirectiveMemento {
  constructor(value, callOnAllChangesDone = false, callOnChange = false) {
    this.value = value;
