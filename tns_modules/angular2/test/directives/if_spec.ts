...
export function main() {
  describe('if directive', () => {
    ...
    if (!IS_DARTIUM) {
      it('should not add the element twice if the condition goes from true to true (JS)', inject([TestBed, AsyncTestCompleter], (tb, async) => {
      ...
      it('should not recreate the element if the condition goes from true to true (JS)', inject([TestBed, AsyncTestCompleter], (tb, async) => {
        var html = '<div><copy-me template="if numberCondition">hello</copy-me></div>';
        tb.createView(TestComponent, {html: html}).then((view) => {
          view.detectChanges();
    ...
    if (IS_DARTIUM) {
      it('should not create the element if the condition is not a boolean (DART)', inject([TestBed, AsyncTestCompleter], (tb, async) => {
        var html = '<div><copy-me template="if numberCondition">hello</copy-me></div>';
        tb.createView(TestComponent, {html: html}).then((view) => {
...
Object.defineProperty(TestComponent, "annotations", {get: function() {
    return [new Component({selector: 'test-cmp'}), new Template({directives: [If]})];
  }});
